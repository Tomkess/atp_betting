...
def features_past_generation(features_creation_function,
                             ...
                             indices):
    """
    Input - match data and feature creation function
    Output - features
...
def features_recent_creation(outcome, match, past_matches):
    ...
    was last match won, 
    ranking of players in last matches,
    number of sets in last matches,
    injuries)
    """
    ## ----- Match information extraction (according to the outcome) ----- ##
    player = match.Winner if outcome == 1 else match.Loser
...
def features_duo_creation(outcome, match, past):
    """
    Input - outcome of a past matches
    Output - features 
    (number of past matches between players, 
    number of wins by player 1, 
    number of wins by player 2,
    ratio of wins by player 1)
    """
    features_duo = []
    ## ----- Match information extraction (according to the outcome) ----- ##
...
def features_general_creation(outcome, match, past_matches):
    """
    Input - outcome of last matches
    Output - comparison of ranking of players, i.e. features based on the ranking
    
    Description: the function returns features based on the ranking of each player
    """
    features_general = []
    ## ----- Match information extraction (according to the outcome) ----- ##
    player1 = match.Winner if outcome == 1 else match.Loser
